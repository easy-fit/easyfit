...
import { SumsubApplicantResponse, WebhookPayload, SumsubWebSDKResponse } from '../types/sumsub.types';
...
export class KYCService {
  ...
  static async createApplicant(userId: string, role: string): Promise<string> {
    try {
      ...
      const response = await this.makeRequest<SumsubApplicantResponse>('post', endpoint, body);
      ...
      throw new AppError('Failed to create Sumsub applicant. Please try again later.', 500);
    }
  }

  static async generateWebSDKLink(userId: string, levelName = this.DEFAULT_LEVEL): Promise<string> {
    try {
      const body = {
        userId,
        levelName,
      ...
      const response = await this.makeRequest<SumsubWebSDKResponse>('post', endpoint, body);

      return response.data.url;
    } catch (error) {
      console.error('Error generating Web SDK link:', error);
      throw new AppError('Failed to generate Web SDK link. Please try again later.', 500);
    }
  }
  ...
  static async handleWebhook(payload: WebhookPayload): Promise<void> {
    try {
      ...
      const user = await UserModel.findOne({
        $or: [{ 'riderInfo.kyc.applicantId': applicantId }, { 'merchantInfo.kyc.applicantId': applicantId }],
      });
      if (!user) {
        return;
      }
      ...
      const result = reviewResult.reviewAnswer === 'GREEN' ? 'verified' : 'rejected';
  ...
  private static buildSumsubHeaders(method: string, endpoint: string, body: any = '') {
    const ts = Math.floor(Date.now() / 1000).toString();
    const rawBody = body ? JSON.stringify(body) : '';
    const message = ts + method.toUpperCase() + endpoint + rawBody;

    const signature = crypto.createHmac('sha256', SUMSUB_CONFIG.SUMSUB_SECRET_KEY).update(message).digest('hex');
  ...
  private static async makeRequest<T>(method: string, endpoint: string, body?: any): Promise<{ data: T }> {
